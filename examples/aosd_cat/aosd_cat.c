...
static void print_usage(const char * argv1) {
    printf(USAGE, argv1, VERSION, REVISION, BUILD_TIME, FONTDESC,
...
parse_opts(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        c = getopt_long (argc, argv, "G:p:A:F:m:f::t::r:g:b:a:x:y:d:e:o:i:hs:S:O:u:wl:B:P:T:",
...
do_input(void) {
    ...
    while (fgets(buf, MAXLINE, INFILE)) {
        ...
        if (*end == '\n') { /* got a full line; replace \n with \0 */
            *end = '\0';
        } else {             /* no full line, append continuation char */
